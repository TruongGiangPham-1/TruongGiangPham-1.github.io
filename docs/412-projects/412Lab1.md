...
<video controls style="width: 100%; height: 80%;">
    <source src="{{ site.baseurl }}/assets/videos/lane_following.mp4" type="video/mp4">
    Your browser does not support the video tag.
</video>
...
The command `dts duckiebot demo ....` ultimately downloaded the lane following script onto the keyboard_control module. When we loaded the keyboard_control GUI, it had option to run.
Interestingly, when we did `rqt_image_view` to see what duckiebot sees during lane following, we found that its camera detected someone's shoe as a yellow tape. However,
...
Increasing speed helped with unexpected or sharp drifts because that helped overcome some of the bumps.

--------------------------------------------------------
# Intrinsic Camera calibration
<img src="{{site.baseurl}}/assets/images/intrinsic.png" alt="Alt text" width="100%" height="auto">
I learn that intrinsic parameters contain pixel scaling, focal length, skew, principle point coordinates, and radial distortion. In order project accurate world POV to 2D image plane of the robot, we need to calibrate these parameters. ([cite](https://github.com/duckietown/lectures/blob/master/1_ideal/25_computer_vision/cv_calibration.pdf)
...
We had issues with the slow and unresponsiveness of the calibration GUI tool. However, the responsiveness of the tool was better on the subsequent run.
...
make the image that the software was built on portable across systems. No difficulties getting the hello world to work. 
...
permission. Recreating the access token with the right permissions fixed it.
